...
`.github/workflows/build-installers.yml` builds them for Windows and Linux and
attaches them to every published release. The same two steps build one locally —
the server executable first, then the installer that carries it:
...
above; Windows-only, silently ignored on Linux. It is generated by
`packaging/make_version_info.py`, which the release build runs first so the
embedded version always matches the release being built; run it yourself
(`python packaging/make_version_info.py 1.2.3`) if you care about the metadata
in a local build. The wizard unpacks the server
...
instead a small bot-managed PR carries the bump, and **merging that PR by hand
is what cuts the release**:
...
- **`.github/workflows/auto-release.yml`** runs on every push to `main`. It
  ...
  Bug Fixes / Breaking Changes). The PR is rewritten from scratch on every
  push, so it always carries the version for everything merged so far. A push
  with nothing releasable (only `chore:`/`docs:`/etc. commits, or the release
  PR's own merge) is a no-op — no branch, PR, or commit is created.
- **`.github/workflows/publish-release.yml`** runs when you merge that
  `release/next` PR. Since branch protection only blocks pushes to the `main`
  ...
  **prerelease** — `1.3.0-rc.1`, then `1.3.0-rc.2` on the next merge, and so
  on — with that version's changelog section as the release notes. It then
  calls `build-installers.yml` to build and attach the installers, so every
...
- **`.github/workflows/promote-release.yml`** is triggered manually
  ...
  opens a `release/promote` PR the same way, carrying the same version
  without the `-rc.N` suffix — `1.3.0`; merging it runs
...
#### Why you merge the release PR yourself

Nothing here auto-merges, and that is load-bearing rather than an oversight.
GitHub does not trigger workflows for events created by `GITHUB_TOKEN`, so a
release PR merged by the bot would fire no `pull_request` event and
`publish-release.yml` would never run. A human merge is a real event. The same
rule is why `publish-release.yml` calls the installer build directly instead of
letting it trigger on `release: published`, which never fires for a release the
bot created.

The alternative — letting the bot push to `main` by adding it to the branch
rule's bypass list — was rejected deliberately: any collaborator with write
access could then push a workflow on a throwaway branch that requests
`permissions: contents: write` and writes straight to `main`, with no review.

This setup needs one repo setting: **Settings → Actions → General → Workflow
permissions → "Allow GitHub Actions to create and approve pull requests"**.
Without it, `auto-release.yml` fails at `gh pr create`.

One wrinkle from the same `GITHUB_TOKEN` rule: **CI does not run on the release
PRs**. If `main` requires status checks, use the admin *"Merge without waiting
for requirements to be met"* button — the diff only ever touches
`pyproject.toml` and `CHANGELOG.md`, and both were already tested on `main` as
part of the PRs that introduced them.
